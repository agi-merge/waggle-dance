...
export const checkEnv = () => {
  ...
  const envMjsContent = fs.readFileSync(join(__dirname, '../apps/nextjs/src/', 'env.mjs'), "utf8");
  const envExampleContent = fs.readFileSync(join(__dirname, '../', '.env.example'), "utf8");
  ...
  console.log(`Found these env vars in ${YELLOW + '.env.example' + RESET}`, envExampleVariables);
  ...
  console.log(`Found these env vars in ${YELLOW + 'env.mjs' + RESET}`, runtimeEnvVariableNames);
